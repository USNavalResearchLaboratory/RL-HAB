...
def gpr_compare():
    ...
    pres = 70 #forecast_subset_era5.ds.level.values[2]
    print(pres)
    ...
    latitudes = era5_slice.ds.latitude.values
    longitudes = era5_slice.ds.longitude.values
    ...
    u_wind_min = np.min(ERA5_u_values) - 2
    u_wind_max = np.max(ERA5_u_values) + 2

    v_wind_min = np.min(ERA5_v_values) - 2
    v_wind_max = np.max(ERA5_v_values) + 2
    ...
    grid_spacing = 10  # Example: use every 5th point in the lat/lon arrays
    ...
    for num_samples in observation_steps:
        ...
        for _ in range(len(X_sample_list)-len(X_era5_grid), num_samples):
            ...
            samp_x = [lat, lon]  # Create a 1D array for the lat-lon pair
            samp_u = era5_slice.ds['u'].sel(latitude=lat, longitude=lon, method="nearest").values
            samp_v = era5_slice.ds['v'].sel(latitude=lat, longitude=lon, method="nearest").values
            X_sample_list.append(samp_x)  # Append lat-lon pair to X_sample
            U_sample_list.append(samp_u)  # Append value to Y_sample
            V_sample_list.append(samp_v)
        #'''
        ...
        noise_levels = np.array([.5 ** 2 if t == 'noisy' else .75 ** 2 for t in types_combined])
        ...
        print("Predicting the GPR)", U_sample.shape)
        print("U_grid", U_sample)
        #asdasda
        ...
        #not sure if this broadcasting is correct, but I think chances are high
        U_std = U_std.reshape(lat_grid.shape)
        V_std = V_std.reshape(lat_grid.shape)
        ...
        angle_diff = ERA5_angles - pred_angles
        ...
        assert U_grid_pred.shape == ERA5_u_values.shape and V_grid_pred.shape == ERA5_v_values.shape, \
            f"Shapes do not match for U?: {U_grid_pred.shape} vs {ERA5_u_values.shape}\nShapes do not match for V?: {V_grid_pred.shape} vs {SYNTH_v_values.shape}"

        # Calculate the difference
        u_prediction_diff = np.abs(U_grid_pred - ERA5_u_values)
        u_mean_diff = np.mean(u_prediction_diff)
        u_std_diff = np.std(u_prediction_diff)

        v_prediction_diff = np.abs(V_grid_pred - ERA5_v_values)
        ...
        img = plt.imshow(U_std, cmap=uv_colormap, origin='lower',
                         extent=[longitudes.min(), longitudes.max(), latitudes.min(), latitudes.max()], vmin=-2,
                         vmax=2)
        cbar = plt.colorbar(img)
        cbar.set_label("Predicted U Wind (m/s)")
        plt.xlabel("Longitude")
        ...
        X_sample_lats = X_sample[:, 0]
        X_sample_lons = X_sample[:, 1]
        plt.scatter(X_sample_lons[len(X_era5_grid):], X_sample_lats[len(X_era5_grid):], color='red', marker='x',
                    label='Observations')
        plt.scatter(X_era5_grid[:, 1], X_era5_grid[:, 0], color='cyan', marker='.', s=2, label='Initialization')
        # print(X_grid[:, 1].tolist())
        # print(X_grid[:, 0])
        ...
        img = plt.imshow(V_std, cmap=uv_colormap, origin='lower',
                         extent=[longitudes.min(), longitudes.max(), latitudes.min(), latitudes.max()], vmin=-2,
                         vmax=2)
        cbar = plt.colorbar(img)
        cbar.set_label("Predicted U Wind (m/s)")
